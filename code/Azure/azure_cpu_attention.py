...
folder_path = '/content/drive/MyDrive/dataset/azure/'
...
for file_name in file_list:
    ...
    # Check if 'avg_cpu' and 'cpu_cores' columns exist
    if 'avg_cpu' not in df.columns or 'cpu_cores' not in df.columns:
        print("Skipping file", file_name, "as it doesn't contain required columns")
        continue
    
    # Convert the 'avg_cpu' column to numerical values
    df['avg_cpu'] = pd.to_numeric(df['avg_cpu'])

    # Extract avg_cpu column
    avg_cpu = df['avg_cpu'].values.reshape(-1, 1)

    # Apply Kalman filter to avg_cpu
    avg_cpu_filtered = apply_kalman_filter(avg_cpu)

    # Scale avg_cpu data
    scaler_avg_cpu = MinMaxScaler(feature_range=(0, 1))
    avg_cpu_scaled = scaler_avg_cpu.fit_transform(avg_cpu_filtered)

    # Extract cpu_cores column
    cpu_cores = df['cpu_cores'].values.reshape(-1, 1)

    # Scale cpu_cores data
    scaler_cpu_cores = MinMaxScaler(feature_range=(0, 1))
    cpu_cores_scaled = scaler_cpu_cores.fit_transform(cpu_cores)

    # Combine scaled avg_cpu and cpu_cores data
    X = np.concatenate((avg_cpu_scaled, cpu_cores_scaled), axis=1)
...
model_save_path = os.path.join(model_save_folder, 'Azure_LSTM_AVG-CPU_w12_60mins.h5')
# Save the model
...
y_pred_inv = scaler_avg_cpu.inverse_transform(y_pred)
y_test_inv = scaler_avg_cpu.inverse_transform(y_test)
