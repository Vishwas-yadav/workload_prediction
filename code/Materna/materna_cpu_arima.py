...
for file_name in file_list:
    ...
    train_size = int(len(cpu_filtered) * 0.8)
    train_data, test_data = cpu_filtered[:train_size], cpu_filtered[train_size:]

    # Fit ARIMA model
    model = ARIMA(train_data, order=(5,1,0)) #change parameters...
    model_fit = model.fit()
