...
for file_name in file_list:
    ...
    # Scale Memory usage data
    scaler_memory = MinMaxScaler(feature_range=(0, 1))
    memory_scaled = scaler_memory.fit_transform(memory_filtered)
    ...
    # Combine scaled Memory usage and cpu_core data
    X = np.concatenate((memory_scaled, core_scaled), axis=1)
...
model_save_path = os.path.join(model_save_folder, 'materna_GRU_MEMORY_w12_60mins.h5')
...
y_pred_inv = scaler_memory.inverse_transform(y_pred)
y_test_inv = scaler_memory.inverse_transform(y_test)
...
plt.plot(y_pred_inv, label='Predicted', color='pink', alpha=0.6)  # Set alpha value to 0.5 for transparency
plt.xlabel('Time Stamp')
plt.ylabel('Memory Utilization %')
plt.legend()
plt.show()
